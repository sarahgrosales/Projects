# Tic Tac Toe Game
# Version 3 includes a counter for each X and O winner
...
line_color = (255, 209, 223)        # Color of straight lines on the game board (divides board into 9 parts)
board = [[None]*3, [None]*3, [None]*3]  # Sets up a 3x3 board in canvas
x_wins = 0                    # Keeps track of number of wins for "X"
o_wins = 0                    # Keeps track of number of wins for "O"
...
def draw_status():
    ...
    font = pg.font.Font(None, 50)                     # Sets font and size for status message
    small_font = pg.font.Font(None, 30)               # Sets font and size for score display
   
    text = font.render(message, 1, (255, 255, 255))   # Renders the status message in white color
    score_text = small_font.render(f"X: {x_wins}  O: {o_wins}", True, (255, 255, 255))  # Renders the score display in white color

    screen.fill((255, 209, 223), (0, 400, 500, 100))             # Fills the area below the game board with black to display status message
    text_rect = text.get_rect(center=(width / 2, 500-50))  # Centers the status message in the area below the game board
    score_rect = score_text.get_rect(center=(width / 2, 400 + 20))  # Centers the score display in the area below the game board
    
    screen.blit(text, text_rect)                           # Displays the status message on the game window
    screen.blit(score_text, score_rect)                    # Displays the score display on the game window
    pg.display.update()                                    # Updates the display to show the changes    

# Main Algorithm
def check_win():
    global board, winner, draw, x_wins, o_wins

    # Checks for winning horizontal rows
    for row in range(0, 3):  
        if ((board[row][0] == board[row][1] == board[row][2]) and (board[row][0] is not None)):
            winner = board[row][0]
            pg.draw.line(screen, (255, 209, 223), (0, (row + 1) * height / 3 - height / 6), (width, (row + 1) * height / 3 - height / 6), 4)
            break
    ...
    for col in range(0, 3):   
        if ((board[0][col] == board[1][col] == board[2][col]) and (board[0][col] is not None)):
            winner = board[0][col]
            pg.draw.line(screen, (255, 209, 223), ((col + 1) * width / 3 - width / 6, 0), ((col + 1) * width / 3 - width / 6, height), 4)
            break

    # Checks for winning diagonal (top-left to bottom-right)
    if (board[0][0] == board[1][1] == board[2][2]) and (board[0][0] is not None):  
        winner = board[0][0]
        pg.draw.line(screen, (255, 209, 223), (50, 50), (350, 350), 4)

    # Checks for winning diagonal (top-right to bottom-left)
    if (board[0][2] == board[1][1] == board[2][0]) and (board[0][2] is not None): 
        winner = board[0][2]
        pg.draw.line(screen, (255, 209, 223), (350, 50), (50, 350), 4)    

    if winner == 'x':      # Keeps track of number of wins for "X" and "O" after checking for win conditions
        x_wins += 1
    elif winner == 'o':
        o_wins += 1
    # Checks for draw 
